...
# ── 연소득 (세전, 만원) ──
ANNUAL_INCOME_P1 = 3300
ANNUAL_INCOME_P2 = 2590
ANNUAL_INCOME_HOUSEHOLD = ANNUAL_INCOME_P1 + ANNUAL_INCOME_P2

# ── 월 세후 수입 (원) ──
NET_INCOME_P1 = 2_500_000
NET_INCOME_P2 = 1_900_000
NET_INCOME_HOUSEHOLD = NET_INCOME_P1 + NET_INCOME_P2

# ── 수입 시점 ──
P2_WORK_START = "2026-07"  # 다음 주 근무 시작
P2_FIRST_PAYCHECK = "2026-08"  # 첫 급여 (익월 지급)
JULY_NET_INCOME = NET_INCOME_P1  # 7월은 현철 월급만
...
SUB_CURSOR_USD = 20
SUB_CURSOR_KRW = 28_000  # $20 × 1,400원 가정
FIXED_SUBSCRIPTION = (
...
# ── [6] 여친 건강보험 (직장가입·8월~, 세전 월 216만) ──
GROSS_INCOME_P2_MONTHLY = 2_160_000
...
KPASS_RATE_YOUTH = 0.30
TRANSIT_LEISURE_RATE = 0.10
FIXED_TRANSPORT = 212_000
FIXED_TRANSPORT_P1 = round(FIXED_TRANSPORT * TRANSIT_P1_GROSS / TRANSIT_GROSS)
FIXED_TRANSPORT_P2 = FIXED_TRANSPORT - FIXED_TRANSPORT_P1
KPASS_NET = int(TRANSIT_GROSS * (1 - KPASS_RATE_YOUTH))
TRANSIT_LEISURE = FIXED_TRANSPORT - KPASS_NET
KPASS_REFUND = TRANSIT_GROSS - KPASS_NET
...
FIXED_P2_EXCLUSIVE = FIXED_PHONE_P2 + FIXED_NHIS_P2 + FIXED_TRANSPORT_P2  # 152,340 (8월~)
FIXED_P1_TOTAL = FIXED_P1_EXCLUSIVE + FIXED_SHARED_HALF  # 844,628
FIXED_P2_TOTAL = FIXED_P2_EXCLUSIVE + FIXED_SHARED_HALF  # 477,340
PARKING_P1 = 850_000  # 버퍼 포함
PARKING_P2 = 480_000

# 7월: 여친 미취업 → 건보 없음
FIXED_JULY_TOTAL = FIXED_MONTHLY_TOTAL - FIXED_NHIS_P2  # 1,235,480
...
FIXED_COST_ITEMS: list[tuple[str, int, str, str]] = [
    ...
    ("Cursor", SUB_CURSOR_KRW, PERSON1_NAME, "$20/월"),
    ("장학금 이자", FIXED_LOAN_SCHOLAR_INTEREST, PERSON1_NAME, "27일 ~2,887"),
    ("카카오 긴급생활", FIXED_LOAN_KAKAO, PERSON1_NAME, "KB 0회·카뱅 확인"),
    ("건강보험(여친)", FIXED_NHIS_P2, PERSON2_NAME, "8월~ 직장가입"),
    ("교통(현철)", FIXED_TRANSPORT_P1, PERSON1_NAME, "K-패스 30%+10%"),
    ("교통(여친)", FIXED_TRANSPORT_P2, PERSON2_NAME, "K-패스 30%+10%"),
]
...
# ── 8월~ 급여일 자산분배 (급여일 +1일 자동이체 권장) ──
# 각 튜플: (항목, 현철, 여친, 목적계좌)
ASSET_ALLOCATION_AUGUST: list[tuple[str, int, int, str]] = [
    ("급여·고정비 (잔액 유지)", 0, 0, "P1/P2-급여·고정비 — 월 85/48만 상시"),
    ("식비(장보기)", FOOD_P1, FOOD_P2, "공동-식비"),
    ("용돈", ALLOWANCE_P1, ALLOWANCE_P2, "P1-용돈 / P2-용돈"),
    ("ISA", SAVE_ISA, 0, "P1-ISA"),
    ("연금저축", SAVE_PENSION_P1, 0, "P1-연금"),
    ("비자·비상", 0, SAVE_VISA_BUFFER_P2, "P2-비자비상"),
    ("집마련", SAVE_HOUSE_P1, SAVE_HOUSE_P2, "P1-CMA / P2-파킹"),
]

# 급여통장 = 고정비통장 (결제 연결·상시 잔액)
SALARY_IS_FIXED_ACCOUNT = True
FIXED_BUFFER_P1 = PARKING_P1  # KB 주거래에 상시 유지
FIXED_BUFFER_P2 = PARKING_P2
HOUSE_ACCOUNT_P1 = "삼성증권 CMA"  # 집마련·RP 이자
HOUSE_ACCOUNT_P2 = "토스뱅크 파킹"  # 집마련·고금리

# ── 집 마련 시 자금 인출 가능 여부 (2031~32 목표) ──
HOUSE_FUND_SOURCES: list[tuple[str, str, str, str]] = [
    ("집마련 CMA·파킹", "✅ 1순위", "자유·T+0~1", "삼성CMA(현철)+토스파킹(여친)·보통예금 대신"),
    ("ISA 서민형", "✅ 2순위", "만기 전 원금 인출", "2029 계약금·이자비과세"),
    ("급여·고정비 KB", "△", "비상 시만", "고정비 버퍼 깨지 않게 최후"),
...
BANK_ACCOUNTS: list[dict[str, str | int]] = [
    {
        ...
        "purpose": f"급여 입금+FBS·지로·카드 결제·상시 {FIXED_BUFFER_P1:,}원",
    },
    {
        "id": "P1-집마련",
        "role": "집마련",
        ...
        "purpose": "RP·MMF 이자·7월 버퍼도 여기",
    },
    {
        "id": "P1-용돈",
        "role": "용돈",
        "owner": PERSON1_NAME,
        "institution": "KB 체크/토스",
    ...
    },
    {
        ...
        "role": "급여·고정비",
        "owner": PERSON2_NAME,
        "institution": "알바 급여 은행",
        "monthly_in": NET_INCOME_P2,
        "purpose": f"급여+건보·교통·통신·상시 {FIXED_BUFFER_P2:,}원",
    },
    {
        "id": "P2-집마련",
        "role": "집마련",
        "owner": PERSON2_NAME,
        "institution": HOUSE_ACCOUNT_P2,
        "monthly_in": SAVE_HOUSE_P2,
        "purpose": "고금리 파킹·월 자동이체",
    },
    {
        "id": "P2-용돈",
        "role": "용돈",
    ...
    },
    {
        ...
        "role": "저축",
        "owner": PERSON2_NAME,
        "institution": "토스 파킹 또는 별도",
        "monthly_in": SAVE_VISA_BUFFER_P2,
        "purpose": "D-10·비상",
    },
    {
        "id": "공동-식비",
...
LIFE_PHASES: list[dict[str, str | int]] = [
    ...
    },
    {
        ...
        "belt_label": "🔴 빡셈",
        "monthly_free": 0,
        "summary": "세후 440만 전부 배분·여가 0",
        "tasks": "집마련 133만·ISA 50·연금 20·용돈 25만 유지",
    },
    {
        "id": "P2",
        "name": "혼인·비자",
...
LIFE_PHASE_RANGES: list[tuple[str, str, str, str, str, int, str]] = [
    ...
    ("P1", "저축전성", "2026-08-01", "2028-06-30", "🔴 빡셈", 0, "저축 208만·여유 0"),
